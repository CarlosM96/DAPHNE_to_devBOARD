#-------------------------------------------------------------
# Generated Example Tcl script for IP 'DAPHNE_TX' (xilinx.com:ip:gtwizard:3.6)
#-------------------------------------------------------------
...
set srcIpDir "/home/daphnelme/Desktop/DAPHNE_TEST/2nd/loopback_v2/loopback_v2.srcs/sources_1/ip/DAPHNE_TX"

# Create project
puts "INFO: \[open_example_project\] Creating new example project..."
create_project -name DAPHNE_TX_ex -force
...
import_ip -files [list [file join $srcIpDir DAPHNE_TX.xci]] -name DAPHNE_TX
reset_target {open_example} [get_ips DAPHNE_TX]
...
generate_target -quiet [_filter_supported_targets {instantiation_template synthesis simulation implementation shared_logic} [get_ips DAPHNE_TX]] [get_ips DAPHNE_TX]
...
add_files -scan_for_includes -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir DAPHNE_TX/example_design/daphne_tx_exdes.v]] \
  [list [file join $srcIpDir DAPHNE_TX/example_design/daphne_tx_gt_frame_check.v]] \
  [list [file join $srcIpDir DAPHNE_TX/example_design/daphne_tx_gt_frame_gen.v]]
...
add_files -quiet -fileset [current_fileset] \
  [list [file join $srcIpDir DAPHNE_TX/example_design/gt_rom_init_tx.dat]]
set_property USED_IN_SIMULATION false [get_files [list [file join $srcIpDir DAPHNE_TX/example_design/gt_rom_init_tx.dat]]]
...
add_files -quiet -fileset [current_fileset -constrset] \
  [list [file join $srcIpDir DAPHNE_TX/example_design/DAPHNE_TX_exdes.xdc]]
...
add_files -quiet -scan_for_includes -fileset [current_fileset -simset] \
  [list [file join $srcIpDir DAPHNE_TX/simulation/daphne_tx_tb.v]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir DAPHNE_TX/simulation/daphne_tx_tb.v]]]
...
add_files -quiet -fileset [current_fileset -simset] \
  [list [file join $srcIpDir DAPHNE_TX/simulation/functional/gt_rom_init_tx.dat]]
set_property USED_IN_SYNTHESIS false [get_files [list [file join $srcIpDir DAPHNE_TX/simulation/functional/gt_rom_init_tx.dat]]]
...
if {[catch {source [file join $srcIpDir DAPHNE_TX/tcl/xci.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errMsg"
  error "ERROR: see log file for details."
  incr returnCode
}
if {[catch {source [file join $srcIpDir DAPHNE_TX/tcl/daphne_tx_partner.tcl]} errMsg]} {
  puts "ERROR: \[open_example_project\] Open Example Project failed: Error encountered while sourcing custom IP example design script."
  puts "$errMsg"
  error "ERROR: see log file for details."
  incr returnCode
